...
class SynchronousCFGParser:
    def __init__(self, grammar: List[Tuple[str, List[ProductionElement], List[ProductionElement]]]):
        ...
        self.non_working_spans = set()
        self.nonterminals = {str(rule[0]) for rule in grammar}

    def can_prove_item(self, item: Item, w1: str, w2: str, count_op: int) -> Tuple[bool, int]:
        ops = 0
        i, j, ip, jp = item.i, item.j, item.ip, item.jp

        # (i, j, ip, jp) in self.non_working_spans:
        #    return False
        #if (i, j, ip, jp) in self.active_spans:
        #    return True
        ...
        if j == i + 1 and jp == ip + 1:
            ...
            for lhs, rhs1, rhs2 in self.grammar:
                ...
                if (lhs == str(item.symbol) and \
                    ...
                    self.active_spans.add((i, j, ip, jp))
                    return True, ops
                else:
                    self.non_working_spans.add((i, j, ip, jp))
    ...
    def parse(self, w1: str, w2: str) -> Tuple[bool, int]:
        """
            Args:
                w1 (str): The first string to be parsed.
                w2 (str): The second string to be parsed.
            Returns:
                Tuple[bool, int]: A tuple containing a boolean indicating if the string pair can be generated by the grammar,
                                    and an integer representing the count of operations performed.
                ...
                for k in range(i, min(n + 1, i + m)): 
                    ...
                    for ip in range(np + 1):
                        for kp in range(ip, min(np + 1, ip + m)):  
                            ...
                            if 'S' in self.get_cell(0, n, 0, np):
                                break
                            ...
                            if k - i + kp - ip == m: 
                                for A in self.nonterminals: 
                                    ...
                                    if A in self.table[i][k][ip][kp]:
                                        ...
                                        can_prove, operations = self.can_prove_item(item, w1, w2, count_op)
                                        count_op += operations
                                        if can_prove:
        ...
        for key in self.table:
            if self.table[key]:
                print(f"Table non-empty cell {key}: {set(self.table[key]), len(set(self.table[key]))}") 

        print(f"Table size:  {len(self.table)}")
        print(f"Active spans: {len(self.active_spans)}, {self.active_spans}")  
        print(f"Non-working spans: {len(self.non_working_spans)}, {self.non_working_spans}")     
        print(f"Count operations: {count_op}")      
...
def main():
    ...
    start = time.time()
    parser1 = SynchronousCFGParser(g2)
    parser2 = SynchronousCFGParser(g2)
    parser3 = SynchronousCFGParser(g2)
    result1 = parser1.parse("cbaaba", "aaaaaa")  # Should return False
    ...
    print(f"Time taken to parse the string: {end-start} seconds")

    start = time.time()
    result3 = parser3.parse("adca", "hgee")  # Should return True
